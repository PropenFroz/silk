...
public class TransaksiBukuController {
    ...
    public List<EntryBuku> getAllEntryTransaksiBuku() {
        List<EntryBuku> entryBukuList = transaksiBukuService.getAllEntryTransaksiBuku();
        List<EntryBukuResponseDTO> responseDTOList = entryBukuList.stream()
                .map(transaksiBukuMapper::toEntryBukuResponseDTO)
